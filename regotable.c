...
{
    printf("Rego=%s value=%d", i->key, i->value);
}
...
// set rego value
int set_rego_value(htab_t *h, rego_t *r, int i)
{
    r->value = i;
    return 0;
}

// For all in the table increment by one if its not -1
void increment_all(htab_t *h)
{
    for (size_t i = 0; i < h->size; ++i)
    {
        if (h->regos[i] != NULL)
        {
            if (h->regos[i]->value != -1)
            {
                set_rego_value(h, h->regos[i], h->regos[i]->value + 1);
            }
        }
    }
}
...
int htab_add(htab_t *h, char *key, int value)
{
    ...
    // Item collision
    if (h->regos[rego])
    {                     // If theres a previous item
        size_t emptySlot; // Save loop value
        for (emptySlot = 0; emptySlot < h->size; emptySlot++)
        {
            if (h->regos[emptySlot] == NULL)
            { // First empty slot
                break;
            }
        }
        newhead->next = h->regos[emptySlot];
        h->regos[emptySlot] = newhead;
        return true;
    }
