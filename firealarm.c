...
int main()
{
	...
	for (int i = 0; i < LEVELS; i++)
	{
		...
		// pthread_create(threads + i, NULL, (void *(*)(void *)) tempmonitor, (void *)i); <-- Original
		// Made it an integer pointer
		pthread_create(threads + i, NULL, (void *(*)(void *))tempmonitor, (void *)(intptr_t)i);
	}
	for (;;)
	{
		if (alarm_active)
...
emergency_mode:
	...
	for (int i = 0; i < ENTRANCES; i++)
	{
		...
		//  Returns the address of the volatile struct
		pthread_create(boomgatethreads + i, NULL, openboomgate, &bg);
	}
	...
	for (;;)
	{
		...
		for (char *p = evacmessage; *p != '\0'; p++)
		{
			for (int i = 0; i < ENTRANCES; i++)
			{
				...
				//  Discard the volatile in the case
